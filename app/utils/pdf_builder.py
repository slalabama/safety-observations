...
# The SL wordmark is loaded once per process from app/static/sl-logo.png,
# converted to a white silhouette (so it reads on the navy header band),
# and cached. File-read failures are non-fatal — the PDF just renders
# without the logo rather than crashing.
...
_sl_white_cache: Optional[bytes] = None
_sl_white_loaded: bool = False


def _whiten_png_bytes(src: bytes) -> bytes:
    """
    Recolor every opaque pixel to white, preserving alpha. Used to render
    the dark-on-transparent SL wordmark as white on the navy header band.
    Falls back to the original bytes if PIL or the conversion fails.
    """
    try:
        ...
        out_img = Image.merge("RGBA", (white, white, white, alpha))
        out = io.BytesIO()
...
def _get_sl_logo_white_bytes() -> Optional[bytes]:
    global _sl_white_cache, _sl_white_loaded
    if _sl_white_loaded:
        return _sl_white_cache
    _sl_white_loaded = True
    try:
        with open(_SL_LOGO_PATH, "rb") as f:
            raw = f.read()
        _sl_white_cache = _whiten_png_bytes(raw)
    except Exception:
        _sl_white_cache = None
    return _sl_white_cache
...
def _draw_header(canvas, doc, kind_label: str, submission_id: int) -> None:
    ...
    # White SL wordmark, left side of the band — sized to fit vertically
    text_x = SIDE_MARGIN  # default x for "Safety 1st" text if logo fails to load
    logo_bytes = _get_sl_logo_white_bytes()
    if logo_bytes:
        try:
            ...
            if iw > 0 and ih > 0:
                ...
                logo_y = PAGE_HEIGHT - HEADER_HEIGHT + (HEADER_HEIGHT - logo_h) / 2.0
                canvas.drawImage(
                    reader,
                    SIDE_MARGIN, logo_y,
                ...
                text_x = SIDE_MARGIN + logo_w + 0.18 * inch
        except Exception:
            pass  # render without the logo rather than failing the whole PDF

    # "Safety 1st" wordmark in white, to the right of the logo
