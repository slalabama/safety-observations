...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-group input, .form-group select { width: 100%; padding: 10px 14px; border: 2px solid #e0e0e0; border-radius: 8px; font-size: 14px; outline: none; transition: border-color 0.2s; font-family: inherit; }
        .form-group input:focus, .form-group select:focus { border-color: #1a2b4a; }
        ...
        .btn-icon { background: none; border: none; cursor: pointer; font-size: 16px; padding: 3px 6px; border-radius: 4px; }
        ...
        .badge-admin { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 600; background: #1a2b4a; color: white; }
        .badge-basic { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 600; background: #e8f0fe; color: #1a2b4a; }
        ...
        .modal { background: white; border-radius: 12px; padding: 24px; width: 440px; box-shadow: 0 20px 60px rgba(0,0,0,0.3); }
        .modal h3 { font-size: 16px; font-weight: 700; color: #1a2b4a; margin-bottom: 20px; }
        .modal-footer { display: flex; gap: 10px; margin-top: 20px; }
        .btn-secondary { flex: 1; padding: 10px; background: #f0f0f0; color: #333; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; }
        .btn-save { flex: 1; padding: 10px; background: #1a2b4a; color: white; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; }
    </style>
</head>
<body>
    <div class="sidebar">
    ...
    <div class="modal-overlay" id="editModal">
        <div class="modal">
            ...
            <div class="form-group">
                <label>Email</label>
                <input type="email" id="editEmail" placeholder="e.g. john@company.com">
            </div>
            <div class="form-group">
        ...
        <div class="content">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div style="font-size:12px;color:#666;margin-bottom:8px;">
                            Example:<br>
                            <code style="font-size:11px;background:#f5f5f5;padding:2px 4px;border-radius:3px;">00001,John Smith,Manufacturing</code>
                        </div>
            ...
            <div class="table-card">
                ...
                <table>
                    ...
                    <tbody id="empTable">
                        <tr><td colspan="6" class="empty-state">Loading employees...</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
    ...
    <script>
        ...
        function renderTable(employees) {
            ...
            tbody.innerHTML = employees.map(e => `
                <tr>
                    ...
                    <td><span class="${e.role === 'admin' ? 'badge-admin' : 'badge-basic'}">${e.role === 'admin' ? 'Admin' : 'Basic'}</span></td>
                    <td style="font-size:12px;color:#666;">${e.email || '<span style="color:#ccc;">—</span>'}</td>
                    <td>
                        <button class="btn-icon" onclick="openEdit(${e.id}, '${e.badge}', '${e.name}', '${e.department||''}', '${e.email||''}', '${e.role}')" title="Edit">✏️</button>
                        <button class="btn-icon" onclick="deleteEmployee(${e.id}, '${e.name}')" title="Delete" style="color:#dc3545;">🗑️</button>
                    </td>
                </tr>
            `).join('');
        }
        ...
        async function saveEdit() {
            ...
            if (!badge || !name) { alert('Badge and name are required'); return; }

            const res = await fetch('/api/users/' + id, {
                method: 'PUT',
                headers: {'Content-Type': 'application/json'},
                credentials: 'include',
                body: JSON.stringify({badge, name, department: dept||null, email: email||null, role})
            });
        ...
        async function addEmployee() {
            ...
            try {
                const res = await fetch('/api/users/add', {
                    ...
                    headers: {'Content-Type': 'application/json'},
                    credentials: 'include',
                    body: JSON.stringify({badge, name: empName, department: dept||null, email: email||null, role})
                });
                ...
                if (res.ok) {
                    ...
                    loadEmployees();
                } else { showAlert('addAlert', 'error', data.detail || 'Failed to add employee'); }
        ...
        async function importCSV(input) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    resultDiv.innerHTML = 'Import complete! Imported: <strong>' + data.imported + '</strong> | Skipped: <strong>' + data.skipped + '</strong>' + (data.duplicates.length > 0 ? '<br>' + data.duplicates.length + ' duplicates skipped' : '');
                    loadEmployees();
                } else {
